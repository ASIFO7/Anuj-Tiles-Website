/**
 * copyright 2024 AsifMohamedMohideen - "https://asifmohamed-portfolio.netlify.app"
 */
...
const letterBoxes = document.querySelectorAll("[data-letter-effect]");
const bannerImage = document.getElementById('hero-banner');
...
const setLetterEffect = function () {
  ...
  for (let i = 0; i < letterBoxes.length; i++) {
    ...
    if (i === activeLetterBoxIndex) {
      ...
      // Change the image when the active letter box is updated
      const newImageSrc = letterBoxes[i].getAttribute('data-image');
      if (newImageSrc) {
